...
from app.interfaces.service_dependencies import get_session_service, get_agent_service, get_file_service
...
@router.delete(
    path="/{session_id}/files/{file_id}",
    response_model=Response[Optional[Dict]],
    summary="删除指定会话文件",
    description="删除指定会话中的文件记录",
)
async def delete_session_file(
    ...
    """删除指定会话中的文件记录以及对象存储文件"""
    await session_service.delete_session_file(session_id, file_id)
    await file_service.delete_file(file_id)
    return Response.success(msg="删除会话文件成功")
