...
class SessionService:
    ...
    async def delete_session_file(self, session_id: str, file_id: str) -> None:
        """删除指定会话中的文件记录，并尽量清理沙箱中的对应文件"""
        logger.info(f"删除会话[{session_id}]文件[{file_id}]")
        async with self._uow:
            session = await self._uow.session.get_by_id(session_id)
        if not session:
            raise RuntimeError(f"当前会话不存在[{session_id}], 请核实后重试")

        target_file = next((file for file in session.files if file.id == file_id), None)
        if not target_file:
            raise NotFoundError(f"当前会话文件不存在[{file_id}]")

        if session.sandbox_id and target_file.filepath:
            sandbox = await self._sandbox_cls.get(session.sandbox_id)
            if sandbox:
                try:
                    await sandbox.delete_file(target_file.filepath)
                except Exception:
                    logger.warning(f"删除沙箱文件失败: {target_file.filepath}")

        async with self._uow:
            await self._uow.session.remove_file(session_id, file_id)
